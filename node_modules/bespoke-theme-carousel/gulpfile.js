...
gulp.task('browserify:lib', ['clean:browserify:lib', 'stylus'], function() {
  return gulp.src('lib/bespoke-theme-carousel.js')
    .pipe(isDemo ? plumber() : through())
    .pipe(browserify({ transform: ['brfs'], standalone: 'bespoke.themes.carousel' }))
    ...
    .pipe(gulp.dest('dist'))
    .pipe(rename('bespoke-theme-carousel.min.js'))
    .pipe(uglify())
    .pipe(header(template([
      '/*! <%= name %> v<%= version %> ',
